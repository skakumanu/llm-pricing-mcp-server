...
description: Runs a feature request through this repo's AI-native SDLC pipeline — Spec, Design, Code, Review (parallel correctness/security/simplification), Test & Release — ending with an open PR against develop. Use when the user asks to build/ship/release a feature end-to-end, or explicitly invokes /release-feature.
---

# /release-feature

Runs one feature request through this repo's phase-agent pipeline end to end: **Spec → Design → Code → Review → Test & Release**, ending with a pushed branch and an open PR against `develop`. This does not merge the PR — per `CLAUDE.md` rule 5, that decision is left to the user.
...
Run the Spec phase as a single subagent (`Agent` tool, `subagent_type: "phase-spec"`, foreground — you need its result before continuing). Give it the feature request verbatim and instruct it to return **only** a JSON object with this exact shape, no prose around it:
...
Parse that JSON. If `openQuestions` is non-empty, surface them to the user and ask before proceeding — do not guess on genuine requirements-level ambiguity.

### 2. Design phase

Run the Design phase as a single subagent (`Agent` tool, `subagent_type: "phase-design"`, foreground). Give it the finalized Spec (including how the user resolved any open questions) and instruct it to return **only** a JSON object with this exact shape:
...
Parse that JSON. If `openQuestions` is non-empty, surface them to the user and ask before proceeding — do not guess on genuine technical/design-level ambiguity.
...
git checkout -b <design.branchName>
```

### 4. Run the Code -> Review -> Test pipeline
...
```json
{
  "spec": "<the parsed Spec object>",
  "design": "<the parsed Design object>",
  "branch": "<design.branchName>",
  "attribution": {
...
- The Spec's problem, target use case, and acceptance criteria.
- The Design's approach and target version.
...
- Which acceptance criteria the Test phase verified (or fixed, if one wasn't initially met).
