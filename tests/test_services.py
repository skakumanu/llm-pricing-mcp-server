...
def test_google_pricing_service():
    """Test Google pricing service returns data."""
...
def test_pricing_aggregator_by_provider():
    ...
    # Test Google filter
    google_pricing = aggregator.get_pricing_by_provider("google")
    assert len(google_pricing) > 0
    assert all(model.provider == "Google" for model in google_pricing)
